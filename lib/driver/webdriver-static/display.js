...
function update(state) {
    for (var boardId = 0; boardId < BOARD_COUNT; boardId++) {
        var start = 16 * 9 * boardId;
        var end = 16 * 9 * (boardId + 1);
...
function updateBoard(boardId, state) {
    ...
    for (var stateIndex = 0; stateIndex < registerOrder.length; stateIndex++) {
        var registerIndex = registerOrder[stateIndex];
        var start = 16 * registerIndex;
        var end = 16 * (registerIndex + 1);

        updateRegister(boardId, registerOrder[stateIndex], state.slice(start, end));
    }
}
...
function updateRegister(boardId, registerId, state) {
    ...
    if (registerId < 8) {
        ...
        for (bitIndex = 0; bitIndex < segmentOrder.length; bitIndex++) {
            updateDisplaySegment(boardId, registerId, segmentOrder[bitIndex], state[bitIndex]);
        }
    }
    else {
        var dpOrder = [
        ...
        for (var dpIndex = 0; dpIndex < 8; dpIndex++) {
            updateDisplaySegment(boardId, dpOrder[dpIndex], 'dp', state[dpIndex]);
        }
        for (bitIndex = 8; bitIndex < 16; bitIndex++) {
            updateLed(boardId, 8 - (bitIndex - 7), state[bitIndex]);
        }
    }
}
...
$(function () {
    ...
    socket.on('frame', function (frame) {
        console.log("received " + frameToText(frame))
        update(frame);
    });
});
