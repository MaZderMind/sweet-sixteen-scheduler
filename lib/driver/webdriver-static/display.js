...
function update(frame) {
    for (var boardId = 0; boardId < BOARD_COUNT; boardId++) {
        var start = DIGITS_PER_ROW_PER_BOARD * boardId;
        var end = DIGITS_PER_ROW_PER_BOARD * (boardId + 1);

        updateRow(boardId, 0, frame[0].slice(start, end));
        updateRow(boardId, 1, frame[1].slice(start, end));
    }
}
